...
class ClipLoss(nn.Module):
    ...
    def __init__(
        ...
        self.world_size = (
            world_size if world_size is not None else torch.distributed.get_world_size()
        )
...
class SigLipLoss(nn.Module):
    ...
    def get_logits(
        ...
        logit_scale,
        logit_bias=None,
        cross_enc_wrapper=None,
    ):
        if cross_enc_wrapper is None:
            score = image_features @ text_features.T
        else:
            score = cross_enc_wrapper(image_features, text_features)
    ...
    def _loss(
        ...
        logits = self.get_logits(
            ...
            logit_scale,
            logit_bias,
            cross_enc_wrapper=cross_enc_wrapper,
        )
...
class SigLipLossWithNegs(nn.Module):
    def __init__(
        self,
        cache_labels=False,
        rank=0,
        ...
        super().__init__()
        self.sigliploss = SigLipLoss(cache_labels, rank, world_size, bidir, use_horovod)
        self.alpha = alpha

    def get_ground_truth(self, device, dtype, num_logits) -> torch.Tensor:
        labels = -torch.ones(num_logits, device=device, dtype=dtype)
        return labels
    ...
    def forward(
        ...
        mask = torch.ones(image_features.size(0), dtype=torch.bool)
        mask[torch.arange(text_features.shape[0]) * group_size] = False
        image_features0 = image_features[mask]
        text_features0 = (
            text_features[:, None]
            .expand(-1, group_size - 1, -1)
            .reshape(-1, text_features.size(-1))
        )

        text_features1 = text_features
        image_features1 = image_features[
            torch.arange(text_features.shape[0]) * group_size
        ]

        siglip_loss = self.sigliploss(
            image_features1, text_features1, logit_scale, logit_bias
        )
        negs_loss = self._loss(image_features0, text_features0, logit_scale, logit_bias)

        return (
            {"siglip_loss": siglip_loss, "negs_loss": negs_loss}
            if output_dict
            else loss
        )
